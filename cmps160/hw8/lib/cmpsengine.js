...
class Scene {
    ...
    initShaders(i) {
        if (this.shaders[i] !== undefined) {
            this.gl.useProgram(this.shaders[i]);
            this.gl.program = this.shaders[i];
            return true;
        }

        var program = createProgram(this.gl, this.geometries[i].v_shader, this.geometries[i].f_shader);
        if (!program) {
            console.log('Failed to create program');
            return false;
        }

        this.shaders[i] = program;
